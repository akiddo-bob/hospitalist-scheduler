...
def analyze_block(block_name, lc_assignments, schedule_by_date, attending):
    ...
    # DEBUG: show Hilditch LCs
    for pname in sorted(provider_lcs.keys()):
        if 'hilditch' in pname.lower() or 'ilditch' in pname.lower():
            print(f'  DEBUG provider_lcs[{pname}] = {provider_lcs[pname]}')
    if unmatched_names:
    ...
    for provider in provider_work_weeks:
        ...
        for i, wk in enumerate(sorted_weeks):
            # Build window: start with weekday dates in this ISO week
            window_dates = set()
            for dt in dates_by_week.get(wk, []):
                if dt.weekday() < 5:
                    window_dates.add(dt)

            # Add following Sat-Sun from the same ISO week if provider works them
            for dt in dates_by_week.get(wk, []):
                if dt.weekday() >= 5:
                    window_dates.add(dt)

            # Add preceding Sat-Sun (from prior ISO week) if provider works them
            # AND the prior ISO week is NOT a weekday work week
